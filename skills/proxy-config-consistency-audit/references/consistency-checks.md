...
- SafeMihomo and Egern Lite deliberately contain a reduced service set while
...
The global inventory requires Egern Full/Lite, Mihomo Mobile/OpenWrt/Safe, Stash and Loon Full. Dynamic discovery also reports additional supported profiles. `checked` (compatibility alias: `scanned`) covers visible definitions and writer structure; it does not run the rule, DNS or kernel validators. `skipped` identifies whole-file generated scopes and retired Loon Lite; the writer-specific validator remains necessary to prove generated bytes. `missing` gives absent canonical profiles and produces exit code 2. Use the rule audit for an Egern Lite unknown-policy check; intentional Lite service reduction is not a parity failure.
